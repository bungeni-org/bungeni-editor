 ...
 * @author Dave
 */
public class JudgementRegionsRestlet extends Restlet {

    static final String HEADER = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n<"+JudgementRegion.PACKAGE_ALIAS+">";
    static final String FOOTER = "\n</"+JudgementRegion.PACKAGE_ALIAS+">";
    IBungeniConnector bungeniConnector = null;

    public JudgementRegionsRestlet(IBungeniConnector bungeniConnector) {
    ...
    public void handle(Request request, Response response) {
        try {
            if (request.getMethod().equals(Method.GET)) {
                List<JudgementRegion> items = bungeniConnector.getJudgementRegions();
                response.setStatus(Status.SUCCESS_OK);
                XStream xStream = new XStream(new DomDriver());
                xStream.alias(JudgementRegion.CLASS_ALIAS, JudgementRegion.class);
