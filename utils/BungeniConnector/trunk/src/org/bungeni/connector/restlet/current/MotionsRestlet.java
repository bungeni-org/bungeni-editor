...
public class MotionsRestlet extends Restlet {
    ...
    static final String HEADER = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n<"+Motion.PACKAGE_ALIAS+">";
    static final String FOOTER = "\n</"+Motion.PACKAGE_ALIAS+">";
    IBungeniConnector bungeniConnector = null;

    public MotionsRestlet(IBungeniConnector bungeniConnector) {
    ...
    public void handle(Request request, Response response) {
        try {
            if (request.getMethod().equals(Method.GET)) {
                List<Motion> items = bungeniConnector.getMotions();
                response.setStatus(Status.SUCCESS_OK);
                XStream xStream = new XStream(new DomDriver());
                xStream.alias(Motion.CLASS_ALIAS, Motion.class);
