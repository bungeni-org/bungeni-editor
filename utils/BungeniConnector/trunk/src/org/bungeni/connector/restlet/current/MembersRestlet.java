...
public class MembersRestlet extends Restlet {
    ...
    static final String HEADER = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n<"+Member.PACKAGE_ALIAS+">";
    static final String FOOTER = "\n</"+Member.PACKAGE_ALIAS+">";
    IBungeniConnector bungeniConnector = null;

    public MembersRestlet(IBungeniConnector bungeniConnector) {
    ...
    public void handle(Request request, Response response) {
        try {
            if (request.getMethod().equals(Method.GET)) {
                List<Member> items = bungeniConnector.getMembers();
                response.setStatus(Status.SUCCESS_OK);
                XStream xStream = new XStream(new DomDriver());
                xStream.alias(Member.CLASS_ALIAS, Member.class);
