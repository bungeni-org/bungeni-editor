 ...
 * @author Dave
 */
public class ActScopesRestlet extends Restlet {

    static final String HEADER = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n<"+ActScope.PACKAGE_ALIAS+">";
    static final String FOOTER = "\n</"+ActScope.PACKAGE_ALIAS+">";
    IBungeniConnector bungeniConnector = null;

    public ActScopesRestlet(IBungeniConnector bungeniConnector) {
    ...
    public void handle(Request request, Response response) {
        try {
            if (request.getMethod().equals(Method.GET)) {
                List<ActScope> items = bungeniConnector.getActScopes();
                response.setStatus(Status.SUCCESS_OK);

                XStream xStream = new XStream(new DomDriver());
                xStream.alias(ActScope.PACKAGE_ALIAS, List.class);
                xStream.alias(ActScope.CLASS_ALIAS, ActScope.class);
