...
<!--
    ...
    This is not a literal template - but is used to auto-generate the real XSLT , it contains freemarker template tags 

    The proprietary URIS are injected into this variable : namespace_uris
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    -->
    <xsl:template match="*[ 
        (namespace-uri() ne 'http://www.metalex.org/1.0') and 
        (namespace-uri() ne 'http://editor.bungeni.org/1.0/anx')
                  and (namespace-uri() ne 'http://www.akomantoso.org/2.0')
          and (namespace-uri() ne 'http://www.proprietary.org')
        ]">
    ...
    <!--
        ...
        The below structure is required by the system - do not edit it unless you 
        know what your are doing.
    -->
    

        
    <xsl:template match="*[namespace-uri() eq 'http://www.akomantoso.org/2.0']">
        <xsl:copy>
            <xsl:apply-templates select="@* | node() " />
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="*[namespace-uri() eq 'http://www.proprietary.org']">
        <xsl:copy>
            <xsl:apply-templates select="@* | node() " />
        </xsl:copy>
    </xsl:template>
