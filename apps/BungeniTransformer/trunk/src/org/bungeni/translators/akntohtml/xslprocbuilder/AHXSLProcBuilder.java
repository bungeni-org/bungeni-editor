package org.bungeni.translators.akntohtml.xslprocbuilder;
        ...
        for (int i = 0; i < elements.getLength(); i++) {
            ...
            if (element.getAttributes().getNamedItem("transformTo").getNodeValue().compareTo("") != 0) {
                ...
                emptyXSLTString = emptyXSLTString.replaceAll("element-to-replace",
                        "akn:" + element.getAttributes().getNamedItem("name").getNodeValue());
                emptyXSLTString = emptyXSLTString.replaceAll("new-element",
                        element.getAttributes().getNamedItem("transformTo").getNodeValue());
                emptyXSLTString = emptyXSLTString.replaceAll("element-class",
                        element.getAttributes().getNamedItem("class").getNodeValue());
                ...
                FileUtility.getInstance().StringToFile(outputDirectory
                        + element.getAttributes().getNamedItem("name").getNodeValue() + ".xsl", emptyXSLTString);

                // create the pipelinestep
                pipelineSteps = pipelineSteps + "\t<xsl:template match=\"akn:"
                                + element.getAttributes().getNamedItem("name").getNodeValue()
                                + "\">\n\t\t<xslt step=\"" + i + "\" name=\""
                                + element.getAttributes().getNamedItem("name").getNodeValue() + "\" href=\""
                                + outputDirectory + element.getAttributes().getNamedItem("name").getNodeValue()
                                + ".xsl" + "\" />\n\t\t<xsl:apply-templates />\n\t</xsl:template>\n\n";
            }
            ...
            // otherwise the element will not be translated
                ...
                emptyXSLTString = emptyXSLTString.replaceAll("<new-element>", "");
                emptyXSLTString =
                    emptyXSLTString.replaceAll("<xsl:attribute name=\"class\">element-class</xsl:attribute>", "");
                emptyXSLTString = emptyXSLTString.replaceAll("other-attributes", "");
                emptyXSLTString = emptyXSLTString.replaceAll("</new-element>", "");
