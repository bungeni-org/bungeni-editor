...
public class StructuralErrorSerialize {
    private static org.apache.log4j.Logger log =
        org.apache.log4j.Logger.getLogger(StructuralErrorSerialize.class.getName());
    File   fLogFile       = null;
    ...
    Date   timeStamp      = null;
    String errorsLogFolderName = "errorlog";

    public StructuralErrorSerialize(String sourceDocUrl) {
        this.sourceDocument = sourceDocUrl;
        File fSourceFile = CommonFileFunctions.convertUrlToFile(sourceDocUrl);

        // returns path to the log folder
        String pathToLogFolder =  fSourceFile.getParentFile().getAbsolutePath() + File.separator + this.errorsLogFolderName ;

        //StructuralRulesConfig.getLogPath();
        ...
        File fLogFolder = new File(pathToLogFolder);
        if (!fLogFolder.exists()) {
            fLogFolder.mkdirs();
        }

        this.logFileFolder = fLogFolder.getAbsolutePath();
